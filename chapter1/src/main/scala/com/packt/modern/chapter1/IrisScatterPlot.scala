...
object PetalScatterPlot extends App {
  ...
  val file = document.save("PetalScatterPlot.html")

  println(s"Saved the chart as ${file.url}")
}

object SepalScatterPlot extends App {
  //First lets load our iris data 
  import BreezeSource._
  import source._

   //Create the plot with the title "Petal length vs Width" 
  val plot = new Plot().title("Iris Sepal Length vs Width")
  ...
  val dataPointRenderer = new GlyphRenderer().data_source(source).glyph(diamond)
    ...
    //Set Data range for the X and the Y Axis
  val xRange = new DataRange1d().sources(sepal_length :: Nil)
  val yRange = new DataRange1d().sources(sepal_width :: Nil)
  plot.x_range(xRange).y_range(yRange)

  //X and Y Axis
  val xAxis = new LinearAxis().plot(plot).axis_label("Sepal Length").bounds((1.0, 7.0))
  val yAxis = new LinearAxis().plot(plot).axis_label("Sepal Width").bounds((0.0, 2.5))
  val xgrid = new Grid().plot(plot).axis(xAxis).dimension(0)
  val ygrid = new Grid().plot(plot).axis(yAxis).dimension(1)
  ...
  val document = new Document(plot)
  val file = document.save("SepalScatterPlot.html")
